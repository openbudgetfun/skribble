...
- Existing Material usage is transitional debt. The current state is tracked in `docs/material-dependency-audit.txt` (regenerate with `dart run tool/audit_material_dependencies.dart`): ~57 files wrap a Material widget ("skin" debt — the real rewrite work), ~37 files only use helpers/constants (mechanical import swaps, do these opportunistically).
