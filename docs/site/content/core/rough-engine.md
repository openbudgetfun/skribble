...
Icon rendering uses the same smooth displacement field for filled contours and authored SVG strokes. Theme-derived icon amplitude multiplies the base offset by `1 + 1.5 * lineWobble`, then by a size factor capped at 2. Gentle therefore keeps its earlier amplitude, while Playful and Expressive become visibly distinct. Explicit icon `drawConfig` values bypass this adjustment. The field removes its linear trend, anchors sampled corners and open-stroke endpoints, and adds opposing bends between anchors. This keeps interior stems upright as well as the whole silhouette. Wavelength grows with the contour's size so enlarged icons do not accumulate extra ripples. Displaced paths are cached for repeat paints.

Open stroke endpoints retain their source tangents. Their first and last sampled segments retain the original path geometry, so wavering does not rotate square or butt caps into adjacent dash gaps.
