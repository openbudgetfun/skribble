...
Map<String, dynamic> uncompressTable(data, Map<String, dynamic> tableEntry) {
  if (tableEntry["compression"] == 'WOFF') {
    var inBuffer = new Uint8List.view(data.buffer, tableEntry["offset"] + 2,
...
Function parseBuffer = (Uint8List buffer, opt) {
  ...
  if (signature == String.fromCharCodes([0, 1, 0, 0]) ||
      signature == 'true' ||
      signature == 'typ1') {
  ...
  font.tables["name"] =
      parseNameTable(nameTable["data"], nameTable["offset"], ltagTable);
  font.names = font.tables["name"];

  if (glyfTableEntry != null && locaTableEntry != null) {
    var shortVersion = indexToLocFormat == 0;
    Map<String, dynamic> locaTable = uncompressTable(data, locaTableEntry);
    var locaOffsets = parseLocaTable(
        locaTable["data"], locaTable["offset"], font.numGlyphs, shortVersion);
    Map<String, dynamic> glyfTable = uncompressTable(data, glyfTableEntry);
    font.glyphs = parseGlyfTable(
        glyfTable["data"], glyfTable["offset"], locaOffsets, font, opt);
  ...
  parseHmtxTable(font, hmtxTable["data"], hmtxTable["offset"],
      font.numberOfHMetrics, font.numGlyphs, font.glyphs, opt);
  ...
  if (gdefTableEntry != null) {
    ...
    font.tables["gdef"] =
        parseGDEFTable(gdefTable["data"], gdefTable["offset"]);
  }

  if (gposTableEntry != null) {
    Map<String, dynamic> gposTable = uncompressTable(data, gposTableEntry);
    font.tables["gpos"] =
        parseGposTable(gposTable["data"], gposTable["offset"]);
    font.position.init();
  }
