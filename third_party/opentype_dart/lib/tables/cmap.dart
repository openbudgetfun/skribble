...
parseCmapTableFormat4(Map<String, dynamic> cmap, p, data, start, offset) {
  ...
  var idRangeOffsetParser =
      new Parser(data, start + offset + 16 + segCount * 6);
...
parseCmapTable(data, start) {
  ...
  if (cmap["format"] == 12) {
    ...
    throw ('Only format 4 and 12 cmap tables are supported (found format ' +
        cmap["format"] +
        ').');
  }
...
addTerminatorSegment(t) {
  t.segments.push({"end": 0xFFFF, "start": 0xFFFF, "delta": 1, "offset": 0});
}
...
makeCmapTable(glyphs) {
  ...
  for (i = 0; i < segCount; i += 1) {
    ...
    if (segment.end <= 65535 && segment.start <= 65535) {
      endCounts
          .add({"name": 'end_' + i, "type": 'USHORT', "value": segment.end});
      startCounts.add(
          {"name": 'start_' + i, "type": 'USHORT', "value": segment.start});
      idDeltas.add(
          {"name": 'idDelta_' + i, "type": 'SHORT', "value": segment.delta});
      idRangeOffsets.add({
    ...
    if (!isPlan0Only && segment.glyphIndex != null) {
      ...
      cmap12Groups.add(
          {"name": 'cmap12End_' + i, "type": 'ULONG', "value": segment.end});
      cmap12Groups.add({
        "name": 'cmap12Glyph_' + i,
        "type": 'ULONG',
