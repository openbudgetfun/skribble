...
parseLookup1(scope) {
  ...
  assertfn(false,
      '0x ${start.toString()}: posformat: ${posformat} GPOS lookup type 1 format must be 1 or 2.');
}
...
parseLookup2(scope) {
  var start = scope.offset + scope.relativeOffset;
  var posFormat = scope.parseUShort();
  assertfn(posFormat == 1 || posFormat == 2,
      '0x' + start.toString() + ': GPOS lookup type 2 format must be 1 or 2.');
  ...
  if (posFormat == 1) {
    ...
    return {
      ...
      "pairSets": scope.parseList(
          Parser.pointer(Parser.list((scope) {
            return {
              // pairValueRecord
    ...
    return {
      ...
      "classRecords": scope.parseList(
          class1Count,
          Parser.list(class2Count, () {
            return {
              "value1": scope.parseValueRecord(valueFormat1),
              "value2": scope.parseValueRecord(valueFormat2)
            };
...
Map<String, dynamic> parseGposTable(data, start) {
  ...
  argument(tableVersion == 1 || tableVersion == 1.1,
